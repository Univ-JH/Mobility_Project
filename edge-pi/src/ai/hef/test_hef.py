...
# 🛠️ 관심 영역(ROI) 비율 설정: 바퀴 앞부분(화면 중앙 하단)
# 가로 30% ~ 70% 구역 / 세로 60% ~ 100% 구역을 판별 기준으로 삼음
ROI_X_RATIO = (0.3, 0.7) 
ROI_Y_RATIO = (0.6, 1.0)
    ...
    with VDevice() as target:
        ...
        with network_group.activate(network_group_params):
            with InferVStreams(network_group, input_vstreams_params, output_vstreams_params) as infer_pipeline:
                print("🚀 실시간 주행 상태 판별을 시작합니다! (종료하려면 화면 클릭 후 'q' 입력)")
                        ...
                        if len(indices) > 0:
                            ...
                            if road_pixels == 0 and sidewalk_pixels == 0:
                                ...
                                try:
                                    ...
                                    pass # 송신 에러가 루프를 멈추지 않게 함
                            else:
                                current_state = "SIDEWALK (인도) - WARNING!"
                                state_color = (0, 0, 255) # 빨강
                                try:
