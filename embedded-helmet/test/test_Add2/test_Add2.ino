...
void loop() {
  ...
  // 각각의 이벤트를 독립적으로 카운트하기 위해 시간 저장 변수 분리
  static unsigned long lastAccEventTime = 0;   // 급가속 마지막 발생 시간
  static unsigned long lastDecEventTime = 0;   // 급정거 마지막 발생 시간
  ...
  // [수정] 마지막 급가속 발생 이후 SUDDEN_LOCK_MS(3초)가 지나야만 재발생 가능
  ...
  // [수정] 마지막 급정거 발생 이후 SUDDEN_LOCK_MS(3초)가 지나야만 재발생 가능
  else if (ax < -SUDDEN_THR && abs(az) > 0.7) {
